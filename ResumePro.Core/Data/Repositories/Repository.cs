...
namespace ResumePro.Core.Data.Repositories;
...
public class Repository<TEntity> : IRepositoryAsync<TEntity> where TEntity : class, IObjectState
{
    ...
    public async Task<int> DeleteManyAsync(List<Guid> parameters)
    {
        var tableName = typeof(TEntity).Name;
        var sql = string.Format("DELETE FROM [{0}] WHERE Id IN ({1})", tableName,
